...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($proveedor_id === '' || $comp_num === '') throw new Exception('Proveedor y número de comprobante son obligatorios.');
        ...
        <div class="col-md-4">
          ...
          <select class="form-select" name="proveedor_id" required>
            ...
            <?php foreach ($proveedores as $p): ?>
              <option value="<?= $p['id'] ?>"><?= e($p['nombre']) ?> (<?= e($p['cuit']) ?>)</option>
            <?php endforeach; ?>
          </select>
          <div class="mt-2">
            <small id="proveedorDatos"></small>
          </div>
        </div>
        <div class="col-md-2">
